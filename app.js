    ...
    overlay.querySelector('#sale-renew-confirm').onclick = async () => {
        ...
        try {
            ...
            syncToGoogleSheet({
                ...
                package: `${newSessions}b/tuần × ${newMonths} tháng`
            });
...
window.renewSaleContract = async function (studentId, studentName) {
    ...
    try {
        ...
        if (st.assignedTeacherId) {
            sendNotification(st.assignedTeacherId, 'contract',
...
function getNextActiveIndex(fixedOrder, currentIdx, debtMap, teachers) {
    ...
    // Chỉ ở bước này nợ mới được tiêu — vì slot này ĐÚNG PHIÊN nhưng bị nợ nên mất lượt
    ...
    // Bước 2: Tìm slot TIẾP THEO sau slot nhận để đặt con trỏ
    // KHÔNG tiêu nợ ở đây — nợ chỉ bị tiêu khi slot đó ĐẾN LƯỢT nhận HĐ (Bước 1 lần sau)
    let checked = 0;
    ...
    while (checked < len) {
        ...
        const teacher = teachers.find(t => t.id === tid);
        if (!teacher || teacher.queuePaused) {
            // GV paused hoặc không tồn tại → bỏ qua hoàn toàn
            idx = (idx + 1) % len;
            checked++;
            continue;
        }
        // Slot active (kể cả có nợ) → đây là slot tiếp theo
        // Nợ sẽ được xử lý khi slot này đến lượt nhận HĐ (Bước 1)
...
window.renewClbAthlete = async function (athleteId, athleteName) {
    if (currentUserRole !== 'ADMIN') return alert('⚠️ Chỉ Admin được gia hạn!');
